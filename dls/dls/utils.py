...
from dls.dls.md import find_macros, markdown_to_html
...
def get_lesson_icon(body, content):
	...
	for macro in macros:
		if macro[0] == "YouTubeVideo" or macro[0] == "Video" or macro[0] == "VimeoVideo":
			return "icon-youtube"
		elif macro[0] == "Quiz":
			return "icon-quiz"
...
def get_tags(course):
	tags = frappe.db.get_value("DLS Course", course, "tags")
...
def get_students(course, batch=None):
	...
	return frappe.get_all("DLS Enrollment", filters, ["member"])
...
def get_reviews(course):
	...
	out_of_ratings = frappe.db.get_all(
		"DocField", {"parent": "DLS Course Review", "fieldtype": "Rating"}, ["options"]
	)
...
def is_certified(course):
	certificate = frappe.get_all(
		"DLS Certificate", {"member": frappe.session.user, "course": course}
	)
	if len(certificate):
...
def get_batch(course, batch_name):
	return frappe.get_all("DLS Batch Old", {"name": batch_name, "course": course})
...
def is_mentor(course, email):
	...
	return frappe.db.count(
		"DLS Course Mentor Mapping", {"course": course, "mentor": email}
	)
...
def get_mentors(course):
	...
	course_mentors = []
	mentors = frappe.get_all("DLS Course Mentor Mapping", {"course": course}, ["mentor"])
	for mentor in mentors:
		member = frappe.db.get_value(
			"User", mentor.mentor, ["name", "username", "full_name", "user_image"]
		)
		member.batch_count = frappe.db.count(
			"DLS Enrollment", {"member": member.name, "member_type": "Mentor"}
		)
...
def is_eligible_to_review(course):
	...
	if frappe.db.count(
		"DLS Course Review", {"course": course, "owner": frappe.session.user}
	):
		return False
	return True
...
def get_initial_members(course):
	members = frappe.get_all("DLS Enrollment", {"course": course}, ["member"], limit=3)
...
def get_signup_optin_checks():
	...
	for check in checks:
		if frappe.db.get_single_value("DLS Settings", check):
			page = frappe.db.get_single_value("DLS Settings", mapper[check].get("page_name"))
...
def handle_notifications(doc, method):
	...
	if topic.reference_doctype not in ["Course Lesson", "DLS Batch"]:
		return
...
def create_notification_log(doc, topic):
	...
	if topic.reference_doctype == "Course Lesson":
		...
		course_title = frappe.db.get_value("DLS Course", course, "title")
		...
		batch_title = frappe.db.get_value("DLS Batch", topic.reference_docname, "title")
...
def notify_mentions_via_email(doc, topic):
	...
	if topic.reference_doctype == "DLS Batch":
...
def get_course_completion_data():
	all_membership = frappe.db.count("DLS Enrollment")
	completed = frappe.db.count("DLS Enrollment", {"progress": ["like", "%100%"]})
...
def is_onboarding_complete():
	...
	course_created = frappe.db.a_row_exists("DLS Course")
	...
	if course_created and chapter_created and lesson_created:
		frappe.db.set_single_value("DLS Settings", "is_onboarding_complete", 1)

	return {
		"is_onboarded": frappe.db.get_single_value("DLS Settings", "is_onboarding_complete"),
		"course_created": course_created,
		"chapter_created": chapter_created,
		"lesson_created": lesson_created,
		"first_course": frappe.get_all("DLS Course", limit=1, order_by=None, pluck="name")[0]
		if course_created
		else None,
	}
...
def get_evaluator(course, batch=None):
	...
	if batch:
		...
		evaluator = frappe.db.get_value("DLS Course", course, "evaluator")
	return evaluator
...
def get_upcoming_evals(student, courses, batch=None):
	...
	for evals in upcoming_evals:
		evals.course_title = frappe.db.get_value("DLS Course", evals.course, "title")
...
def check_multicurrency(amount, currency, country=None, amount_usd=None):
	settings = frappe.get_single("DLS Settings")
...
def apply_gst(amount, country=None):
	gst_applied = 0
	apply_gst = frappe.db.get_single_value("DLS Settings", "apply_gst")
...
def create_membership(course, payment):
	membership = frappe.new_doc("DLS Enrollment")
	membership.update(
		{"member": frappe.session.user, "course": course, "payment": payment.name}
	)
	membership.save(ignore_permissions=True)
	return f"/dls/courses/{course}/learn/1-1"
...
def update_course_filters(filters):
	...
	if filters.get("enrolled"):
		enrolled_courses = frappe.get_all(
			"DLS Enrollment", {"member": frappe.session.user}, pluck="course"
		)
...
def get_lesson(course, chapter, lesson):
	...
	course_info = frappe.db.get_value(
		"DLS Course", course, ["title", "paid_certificate"], as_dict=1
	)
...
def get_batch_details(batch):
	batch_students = frappe.get_all(
		"DLS Batch Enrollment", {"batch": batch}, pluck="member"
	)
	if (
		not frappe.db.get_value("DLS Batch", batch, "published")
...
def categorize_batches(batches):
	...
	for batch in batches:
		...
		if frappe.session.user != "Guest":
			if frappe.db.exists(
				"DLS Batch Enrollment", {"member": frappe.session.user, "batch": batch.name}
			):
...
def get_question_details(question):
	...
	question_details = frappe.db.get_value("DLS Question", question, fields, as_dict=1)
...
def get_assessments(batch, member=None):
	...
	for assessment in assessments:
		...
		elif assessment.assessment_type == "DLS Quiz":
...
def get_assignment_details(assessment, member):
	assessment.title = frappe.db.get_value(
		"DLS Assignment", assessment.assessment_name, "title"
	)
...
def get_quiz_details(assessment, member):
	assessment_details = frappe.db.get_value(
		"DLS Quiz", assessment.assessment_name, ["title", "passing_percentage"], as_dict=1
	)
...
def get_batch_students(batch):
	...
	students_list = frappe.get_all(
		"DLS Batch Enrollment", filters={"batch": batch}, fields=["member", "name"]
	)
	...
	for student in students_list:
		...
		for course in batch_courses:
			progress = frappe.db.get_value(
				"DLS Enrollment", {"course": course.course, "member": student.member}, "progress"
			)
...
def has_submitted_assessment(assessment, assessment_type, member=None):
	...
	if assessment_type == "DLS Assignment":
		doctype = "DLS Assignment Submission"
		docfield = "assignment"
		fields = ["status"]
		not_attempted = "Not Attempted"
	elif assessment_type == "DLS Quiz":
	...
	if attempt:
		...
		if assessment_type == "DLS Quiz":
			result = "Failed"
			passing_percentage = frappe.db.get_value(
				"DLS Quiz", assessment, "passing_percentage"
			)
...
def get_roles(name):
	...
	return {
		...
		"dls_student": has_student_role(name),
	}


def publish_notifications(doc, method):
	frappe.publish_realtime(
		"publish_dls_notifications", user=doc.for_user, after_commit=True
	)
...
def enroll_in_course(course, payment_name):
	if not frappe.db.exists(
		"DLS Enrollment", {"member": frappe.session.user, "course": course}
	):
		enrollment = frappe.new_doc("DLS Enrollment")
		payment = frappe.db.get_value(
			"DLS Payment", payment_name, ["name", "source"], as_dict=True
		)
...
def enroll_in_batch(batch, payment_name=None):
	if not frappe.db.exists(
		"DLS Batch Enrollment", {"batch": batch, "member": frappe.session.user}
	):
		batch_doc = frappe.db.get_value(
			"DLS Batch", batch, ["name", "seat_count"], as_dict=True
		)
		students = frappe.db.count("DLS Batch Enrollment", {"batch": batch})
		...
		new_student = frappe.new_doc("DLS Batch Enrollment")
		new_student.update(
			{
				"member": frappe.session.user,
				"batch": batch,
...
def get_programs():
	if (
		...
		programs = frappe.get_all("DLS Program", fields=["name"])
	else:
		programs = frappe.get_all(
			"DLS Program Member", {"member": frappe.session.user}, ["parent as name", "progress"]
		)

	for program in programs:
		program_courses = frappe.get_all(
			"DLS Program Course", {"parent": program.name}, ["course"], order_by="idx"
		)
		program.courses = []
		previous_progress = 0
		...
		program.members = frappe.db.count("DLS Program Member", {"parent": program.name})
...
def enroll_in_program_course(program, course):
	...
	if enrollment:
		enrollment = frappe.db.get_value(
			"DLS Enrollment", enrollment, ["name", "current_lesson"], as_dict=1
		)
		enrollment.current_lesson = get_lesson_index(enrollment.current_lesson)
		return enrollment

	program_courses = frappe.get_all(
		"DLS Program Course", {"parent": program}, ["course", "idx"], order_by="idx"
	)
	current_course_idx = [
		program_course.idx
...
def get_batches(filters=None, start=0, page_length=20, order_by="start_date"):
	...
	if filters.get("enrolled"):
		enrolled_batches = frappe.get_all(
			"DLS Batch Enrollment", {"member": frappe.session.user}, pluck="batch"
		)
...
def get_batch_card_details(batches):
	for batch in batches:
		...
		students_count = frappe.db.count("DLS Batch Enrollment", {"batch": batch.name})
...
def persona_captured():
	frappe.db.set_single_value("DLS Settings", "persona_captured", 1)
