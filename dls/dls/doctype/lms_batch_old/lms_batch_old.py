...
from dls.dls.doctype.dls_enrollment.dls_enrollment import create_membership
from dls.dls.utils import is_mentor


class DLSBatchOld(Document):
	def validate(self):
		pass
	...
	def is_member(self, email, member_type=None):
		...
		return frappe.db.exists("DLS Enrollment", filters)
	...
	def get_membership(self, email):
		...
		return frappe.get_doc("DLS Enrollment", name)
...
def switch_batch(course_name, email, batch_name):
	...
	membership = frappe.get_last_doc(
		"DLS Enrollment", filters={"course": course_name, "member": email}
	)

	batch = frappe.get_doc("DLS Batch Old", batch_name)
	if not batch:
	...
	old_batch = frappe.get_doc("DLS Batch Old", membership.batch_old)
