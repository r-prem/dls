...
class DLSQuiz(Document):
	...
	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "DLS Quiz")
...
def quiz_summary(quiz, results):
	...
	submission = frappe.new_doc("DLS Quiz Submission")
...
def get_question_details(question):
	if frappe.db.exists("DLS Quiz Question", question):
		...
		return frappe.db.get_value("DLS Quiz Question", question, fields, as_dict=1)
	return
