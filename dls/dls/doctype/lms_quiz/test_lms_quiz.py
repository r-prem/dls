...
class TestDLSQuiz(unittest.TestCase):
	@classmethod
	def setUpClass(cls) -> None:
		frappe.get_doc(
			{"doctype": "DLS Quiz", "title": "Test Quiz", "passing_percentage": 90}
		).save(ignore_permissions=True)

	def test_with_multiple_options(self):
		question = frappe.new_doc("DLS Question")
	...
	def tearDownClass(cls) -> None:
		frappe.db.delete("DLS Quiz", "test-quiz")
		frappe.db.delete("DLS Question")
