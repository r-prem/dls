...
class Cohort(Document):
	def get_url(self):
		return f"{frappe.utils.get_url()}/dls/courses/{self.course}/cohorts/{self.slug}"
	...
	def get_subgroups(self, include_counts=False, sort_by=None):
		...
		if include_counts:
			...
			students = self._get_subgroup_counts("DLS Enrollment")
	...
	def get_stats(self):
		return {
			...
			"students": self._get_count("DLS Enrollment"),
