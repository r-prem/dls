...
from dls.dls.utils import is_certified
from frappe.email.doctype.email_template.email_template import get_email_template
from frappe.model.naming import make_autoname


class DLSCertificate(Document):
	def validate(self):
	...
	def send_mail(self):
		...
		custom_template = frappe.db.get_single_value("DLS Settings", "certification_template")
...
def create_certificate(course):
	...
	if certificate:
		return frappe.db.get_value(
			"DLS Certificate", certificate, ["name", "course", "template"], as_dict=True
		)
