...
class DLSAssignmentSubmission(Document):
	def validate(self):
...
def upload_assignment(
	...
	assignment_details = frappe.db.get_value(
		"DLS Assignment", assignment, ["type", "grade_assignment"], as_dict=1
	)
	...
	if submission:
		doc = frappe.get_doc("DLS Assignment Submission", submission)
	else:
		doc = frappe.get_doc(
			{
...
def grade_assignment(name, result, comments):
	doc = frappe.get_doc("DLS Assignment Submission", name)
	doc.status = result
