...
class DLSCourse(Document):
	...
	def on_payment_authorized(self, payment_status):
		if payment_status in ["Authorized", "Completed"]:
			update_payment_record("DLS Course", self.name)

	def send_email_to_interested_users(self):
		interested_users = frappe.get_all(
			"DLS Course Interest", {"course": self.name}, ["name", "user"]
		)
		subject = self.title + " is available!"
		args = {
			"title": self.title,
			"course_link": f"/dls/courses/{self.name}",
		...
		for user in interested_users:
			...
			frappe.db.set_value("DLS Course Interest", user.name, "email_sent", True)

	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "DLS Course")
	...
	def has_mentor(self, email):
		...
		mapping = frappe.get_all(
			"DLS Course Mentor Mapping", {"course": self.name, "mentor": email}
		)
		return mapping != []
	...
	def add_mentor(self, email):
		...
		doc = frappe.get_doc(
			{"doctype": "DLS Course Mentor Mapping", "course": self.name, "mentor": email}
		)
		doc.insert()
	...
	def get_student_batch(self, email):
		...
		return batch_name and frappe.get_doc("DLS Batch Old", batch_name)

	def get_batches(self, mentor=None):
		batches = frappe.get_all("DLS Batch Old", {"course": self.name})
		if mentor:
			# TODO: optimize this
			memberships = frappe.db.get_all("DLS Enrollment", {"member": mentor}, ["batch_old"])
	...
	def get_all_memberships(self, member):
		all_memberships = frappe.get_all(
			"DLS Enrollment", {"member": member, "course": self.name}, ["batch_old"]
		)
		for membership in all_memberships:
			membership.batch_title = frappe.db.get_value(
				"DLS Batch Old", membership.batch_old, "title"
			)
...
def reindex_exercises(doc):
	...
	course = frappe.get_doc("DLS Course", course_data["name"])
