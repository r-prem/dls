...
frappe.ui.form.on("DLS Batch", {
	onload: function (frm) {
		...
		frm.set_query("assessment_type", "assessment", function () {
			let doctypes = ["DLS Quiz", "DLS Assignment"];
			return {
				filters: {
					name: ["in", doctypes],
				},
	...
	refresh: (frm) => {
		frm.add_web_link(
			`/dls/batches/details/${frm.doc.name}`,
			"See on website"
		);
	},
});
...
const set_timetable = (frm) => {
	if (frm.doc.timetable_template) {
		...
		frappe.call({
			...
			args: {
				doctype: "DLS Batch Timetable",
				parent: "DLS Timetable Template",
				fields: [
					"reference_doctype",
					"reference_docname",
					"day",
				...
				filters: {
					...
					parenttype: "DLS Timetable Template",
				},
				order_by: "idx",
			},
			callback: (data) => {
...
const set_default_legends = (frm) => {
	const data = [
		...
		{
			reference_doctype: "DLS Quiz",
			label: "DLS Quiz",
			color: "#39E4A5",
		},
		{
			reference_doctype: "DLS Assignment",
			label: "DLS Assignment",
			color: "#ECAD4B",
		},
		{
			reference_doctype: "DLS Live Class",
			label: "DLS Live Class",
			color: "#bb8be8",
		},
	];
