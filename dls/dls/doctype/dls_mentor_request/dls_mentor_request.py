...
class DLSMentorRequest(Document):
	def on_update(self):
	...
	def create_course_mentor_mapping(self):
		mapping = frappe.get_doc(
			{
				"doctype": "DLS Course Mentor Mapping",
				"mentor": self.member,
				"course": self.course,
			}
		)
	...
	def send_status_change_email(self):
		...
		course_details = frappe.db.get_value(
			"DLS Course", self.course, ["owner", "title"], as_dict=True
		)
		message = frappe.render_template(
			email_template.response,
			{
	...
	def get_email_template(self, template_name):
		template = frappe.db.get_single_value("DLS Settings", template_name)
		if template:
