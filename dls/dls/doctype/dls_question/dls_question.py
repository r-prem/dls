...
from dls.dls.utils import has_course_instructor_role, has_course_moderator_role


class DLSQuestion(Document):
	def validate(self):
...
def update_question_title(question):
	if not question.is_new():
		question_rows = frappe.get_all(
			"DLS Quiz Question", {"question": question.name}, pluck="name"
		)

		for row in question_rows:
			frappe.db.set_value("DLS Quiz Question", row, "question_detail", question.question)
...
def get_question_details(question):
	...
	return frappe.db.get_value("DLS Question", question, fields, as_dict=1)
