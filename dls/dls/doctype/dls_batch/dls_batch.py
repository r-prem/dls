...
class DLSBatch(Document):
	...
	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "DLS Batch")
	...
	def validate_duplicate_courses(self):
		...
		if len(duplicates):
			title = frappe.db.get_value("DLS Course", next(iter(duplicates)), "title")
			frappe.throw(
	...
	def validate_membership(self):
		members = frappe.get_all("DLS Batch Enrollment", {"batch": self.name}, pluck="member")
		for course in self.courses:
			for member in members:
				if not frappe.db.exists(
					"DLS Enrollment", {"course": course.course, "member": member}
				):
	...
	def validate_seats_left(self):
		students = frappe.db.count("DLS Batch Enrollment", {"batch": self.name})
	...
	def on_payment_authorized(self, payment_status):
		if payment_status in ["Authorized", "Completed"]:
			update_payment_record("DLS Batch", self.name)
...
def create_batch(
	...
	frappe.only_for("Moderator")
	if name:
		doc = frappe.get_doc("DLS Batch", name)
	else:
		doc = frappe.get_doc({"doctype": "DLS Batch"})
...
def get_batch_timetable(batch):
	...
	show_live_class = frappe.db.get_value("DLS Batch", batch, "show_live_class")
	if show_live_class:
...
def get_live_classes(batch):
	...
	for class_ in live_classes:
		...
		class_.reference_doctype = "DLS Live Class"
...
def get_timetable_details(timetable):
	for entry in timetable:
		...
		elif entry.reference_doctype == "DLS Quiz":
			entry.url = "/quizzes"
			details = get_quiz_details(assessment, frappe.session.user)
			entry.update(details)

		elif entry.reference_doctype == "DLS Assignment":
...
def is_milestone_complete(idx, batch):
	...
	for row in previous_rows:
		...
		if row.reference_doctype == "DLS Quiz":
		...
		if row.reference_doctype == "DLS Assignment":
			if not frappe.db.exists(
...
def send_batch_start_reminder():
	...
	for batch in batches:
		students = frappe.get_all(
			"DLS Batch Enrollment", {"batch": batch.name}, ["member", "member_name"]
		)
