...
class CourseLesson(Document):
	...
	def validate_quiz_id(self):
		if self.quiz_id and not frappe.db.exists("DLS Quiz", self.quiz_id):
	...
	def save_lesson_details_in_quiz(self, content):
		...
		for block in content.get("blocks"):
			if block.get("type") == "quiz":
				...
				if not frappe.db.exists("DLS Quiz", quiz):
...
def save_progress(lesson, course):
	membership = frappe.db.exists(
		"DLS Enrollment", {"course": course, "member": frappe.session.user}
	)
	if not membership:
		return 0

	frappe.db.set_value("DLS Enrollment", membership, "current_lesson", lesson)
	already_completed = frappe.db.exists(
		"DLS Course Progress", {"lesson": lesson, "member": frappe.session.user}
	)
	...
	enrollment = frappe.get_doc("DLS Enrollment", membership)
...
def capture_progress_for_analytics(progress, course):
	if progress in [25, 50, 75, 100]:
		capture("course_progress", "dls", properties={"course": course, "progress": progress})
...
def get_quiz_progress(lesson):
	...
	for quiz in quizzes:
		passing_percentage = frappe.db.get_value("DLS Quiz", quiz, "passing_percentage")
