...
def is_batch_sold_out(payment):
	if payment.payment_for_document_type == "DLS Batch":
		seat_count = frappe.get_cached_value(
			"DLS Batch", payment.payment_for_document, "seat_count"
		)
		number_of_students = frappe.db.count(
			"DLS Batch Enrollment", {"batch": payment.payment_for_document}
		)
...
def send_mail(payment):
	...
	custom_template = frappe.db.get_single_value(
		"DLS Settings", "payment_reminder_template"
	)
	...
	args = {
		...
		"link": f"/dls/billing/{ payment.payment_for_document_type.split(' ')[-1].lower() }/{ payment.payment_for_document }",
	}
