...
def award(doc, member):
	...
	assignment = frappe.new_doc("DLS Badge Assignment")
	assignment.update(
		{
			"badge": doc.name,
			"member": member,
...
def process_badges(doc, state):
	...
	for d in frappe.cache_manager.get_doctype_map(
		"DLS Badge", doc.doctype, dict(reference_doctype=doc.doctype, enabled=1)
	):
		frappe.get_doc("DLS Badge", d.get("name")).apply(doc)
