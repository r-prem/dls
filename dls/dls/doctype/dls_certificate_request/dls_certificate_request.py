...
from dls.dls.utils import get_evaluator
import json


class DLSCertificateRequest(Document):
	def validate(self):
		self.set_evaluator()
	...
	def validate_if_existing_requests(self):
		...
		for req in existing_requests:
			if (
				...
				course_title = frappe.db.get_value("DLS Course", req.course, "title")
				frappe.throw(
	...
	def validate_evaluation_end_date(self):
		if self.batch_name:
			evaluation_end_date = frappe.db.get_value(
				"DLS Batch", self.batch_name, "evaluation_end_date"
			)
...
def schedule_evals():
	if frappe.db.get_single_value("DLS Settings", "send_calendar_invite_for_evaluations"):
...
def update_meeting_details(eval, event, calendar):
	...
	event.reload()
	frappe.db.set_value(
		"DLS Certificate Request", eval.name, "google_meet_link", event.google_meet_link
	)
...
def create_certificate_request(
	...
	is_member = frappe.db.exists(
		{"doctype": "DLS Enrollment", "course": course, "member": frappe.session.user}
	)

	if not is_member:
		return
	eval = frappe.new_doc("DLS Certificate Request")
	eval.update(
		{
			"course": course,
...
@frappe.whitelist()
def create_dls_certificate_evaluation(source_name, target_doc=None):
	doc = get_mapped_doc(
		"DLS Certificate Request",
		source_name,
		{"DLS Certificate Request": {"doctype": "DLS Certificate Evaluation"}},
		target_doc,
	)
	return doc
...
def mark_eval_as_completed():
	...
	for req in requests:
		if req.date < getdate():
			frappe.db.set_value("DLS Certificate Request", req.name, "status", "Completed")
