...
class DLSCourseReview(Document):
	def validate(self):
		self.validate_if_already_reviewed()

	def validate_if_already_reviewed(self):
		if frappe.db.exists(
			"DLS Course Review", {"course": self.course, "owner": self.owner}
		):
...
def submit_review(rating, review, course):
	...
	frappe.get_doc(
		{"doctype": "DLS Course Review", "rating": rating, "review": review, "course": course}
