...
class CohortSubgroup(Document):
	...
	def get_url(self):
		...
		return f"{frappe.utils.get_url()}/dls/courses/{self.course}/subgroups/{cohort.slug}/{self.slug}"

	def get_invite_link(self):
		cohort = frappe.get_doc("Cohort", self.cohort)
		return f"{frappe.utils.get_url()}/dls/courses/{self.course}/join/{cohort.slug}/{self.slug}/{self.invite_code}"

	def has_student(self, email):
		"""Check if given user is a student of this subgroup."""
		q = {"doctype": "DLS Enrollment", "subgroup": self.name, "member": email}
