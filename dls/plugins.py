...
behaviour some parts of the dls app.
...
class LiveCodeExtension(PageExtension):
	def render_header(self):
		livecode_url = frappe.get_value("DLS Settings", None, "livecode_url")
...
def quiz_renderer(quiz_name):
	...
	for question in questions:
		details = frappe.db.get_value("DLS Question", question.question, fields, as_dict=1)
		details["marks"] = question.marks
		quiz.questions.append(details)

	no_of_attempts = frappe.db.count(
		"DLS Quiz Submission", {"owner": frappe.session.user, "quiz": quiz_name}
	)
...
def exercise_renderer(argument):
	exercise = frappe.get_doc("DLS Exercise", argument)
...
def show_custom_signup():
	...
	if settings.custom_signup_content or settings.user_category:
		return "dls/templates/signup-form.html"
