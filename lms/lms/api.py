...
def is_paid_courses_enabled():
	return frappe.db.get_single_value('LMS Settings', 'enable_paid_courses')


@frappe.whitelist(allow_guest=True)
def is_ratings_allowed():
	return frappe.db.get_single_value('LMS Settings', 'ratings_allowed')


@frappe.whitelist(allow_guest=True)
def is_enrolled_count_visible():
	return frappe.db.get_single_value('LMS Settings', 'show_enrolled_count')


@frappe.whitelist(allow_guest=True)
def is_questions_active():
	return frappe.db.get_single_value('LMS Settings', 'questions_active')


@frappe.whitelist(allow_guest=True)
def is_instructor_display_enabled():
	return frappe.db.get_single_value('LMS Settings', 'display_instructor')


@frappe.whitelist(allow_guest=True)
def get_lesson_completion_time():
	return frappe.db.get_single_value('LMS Settings', 'lesson_completion_time')
